...
let activityListAction = function (initializer) {
    ...
    const startWorkbench = function () {
        var info = $(this).data("appInfo");
        info.goBack = function () {
            activityListAction(initializer);
        };
        import("/js/workbench/workbench.js").then((module) => {
            module.workbenchAction(info)
    ...
    const renderActivities = function (container, list) {
        let workbenchActivity = null
        const buttonbar = $("<div class='button-bar'>").css(
            {display: "flex"}).appendTo(container);
        if (list.use_new_api) {
            newApiChk = $('<input type="checkbox" />');
            $("<label class='switch'>").appendTo(buttonbar).append(
                newApiChk).append($("<span class='slider'>")).append(
                $("<span class='text'>").text("Load custom properties"));
            workbenchActivity = adbDevice && list.find(isWorkbenchActivity)
        } else {
            newApiChk = null;
        }
        ...
        if (adbDevice) {
            $("<button>").text("Mirror Display").appendTo(buttonbar)
                .click(deviceMirrorAction);
        }

        if (workbenchActivity) {
            $("<button>").text("Workbench").appendTo(buttonbar)
                .data("appInfo", workbenchActivity)
        ...
        for (let i = 0; i < list.length; i++) {
            const l = list[i];
            if (isWorkbenchActivity(l)) {
                continue;
            }
            const entry = $("<div>").data("appInfo", l).appendTo(container).click(
                startHView).addClass("entry");

            if (list.hasIcons) {
                const icon = $('<div class="icon">').appendTo(entry).attr(
                    "icon-pid", l.pid);
