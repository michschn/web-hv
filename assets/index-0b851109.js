...
$(function () {
    ...
    $("#device-picker").click(function () {
        handleSelectDevice(
            navigator.usb.requestDevice({filters: [ADB_DEVICE_FILTER]}));
    });
    ...
    if (urlParams.get("mode") == "mirror") {
        ...
        import("/js/workbench/workbench.js").then((module) => {
            module.workbenchAction()
        })
    } else if (urlParams.get("mode") == "workbench") {
        // Switch to mirror mode
        $("#main-title-wrapper").html("<h2>Workbench</h2>");
        var workbenchActivityFound = false
        activityListAction = function (initializer) {
            initializer({
                windowsLoaded: async function (list) {
                    if (workbenchActivityFound) {
                        return
                    }
                    const {
                        isWorkbenchActivity,
                        workbenchAction
                    } = await import("/js/workbench/workbench.js")
                    let workbenchActivity = list.find(isWorkbenchActivity)
                    if (workbenchActivity) {
                        workbenchActivityFound = true
                        workbenchAction(workbenchActivity)
                    }
                },
                jdwpError: console.error,
                windowsError: console.error,
                iconLoaded: function () {
                }
            });
